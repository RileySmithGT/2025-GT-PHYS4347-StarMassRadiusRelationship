...
m_n_MeV = physical_constants['neutron mass energy equivalent in MeV'][0]
m_n = physical_constants['neutron mass'][0] * 1e3  # neutron mass [g]

# from O&V 1939
term = h / (m_n * c)
r_tilde = (1 / np.pi) * (term**1.5) * (c / np.sqrt(m_n * G))
M_tilde = (1 / np.pi) * (term**1.5) * (c**3 / np.sqrt(m_n * G**3))
...
def build_eos_table(coefs, n_max=2.5, N=1000):
    ...
    for i, ni in enumerate(n):
        ...
        eps_phys = (ni * fm3_to_cm3) * (E_n + m_n_MeV) * MeV_to_erg + m_n
...
def integrate_tov(nc, n_grid, P_t, eps_t, dpdn_t, h=1e-4):
    ...
    while True:
        ...
        k1 = derivs(R, u, t)
        k2 = (
            derivs(R + 0.5 * h, u + 0.5 * h * k1[0], t + 0.5 * h * k1[1])
            if k1
            else None
        )
        k3 = (
            derivs(R + 0.5 * h, u + 0.5 * h * k2[0], t + 0.5 * h * k2[1])
            if k2
            else None
        )
